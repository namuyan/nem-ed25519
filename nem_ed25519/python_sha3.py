...
class Keccak:
    ...
    """
        ...
        if ((n - 8) <= l <= (n - 2)):
            ...
            my_byte = "%02X" % my_byte
            my_string = my_string[0:nr_bytes_filled * 2] + my_byte.encode('utf-8')
        else:
            if (nbr_bits_filled == 0):
                my_byte = 0
            else:
    ...
    def update(self, arg):
        ...
        if type(arg) is str:
            self.buffered_data += arg.encode('utf-8')
        else:
            self.buffered_data += binascii.hexlify(arg)
